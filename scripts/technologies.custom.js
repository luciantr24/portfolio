const renderTechologiesSectionData = () => {
    ...
    technologies.then(result => result.json())
    .then(data => {
        const skillsSection = document.getElementById('skills-section');
        data.forEach(element => {
            const mainTechnologiesSection = document.createElement('div');
            mainTechnologiesSection.className = "col-lg-3 col-md-6 text-center";

            const second_col = document.createElement('div');
            second_col.className = "mt-5";

            const icon = document.createElement('i');
            icon.className = `fas fa-4x ${getIconBasedOnType(element.name)} text-primary mb-4`;;
